 ...
 * @author Dinesh 
 */
public class BalconyTwo implements VenueLevel{
	private static Integer levelValue;
	private static Integer price;
	private static Integer rows;
	private static Integer seatsPerRow;
	private static BalconyTwo venueLevel = null;
	private static Vector<Seat> balconyTwoSeatsList;
	...
	private BalconyTwo() {
		...
		rows = 15;
		seatsPerRow = 100;
		balconyTwoSeatsList = new Vector<Seat>();
		lock = new ReentrantLock(true);
		createBalconyTwoSeatsList();
	}
	
	/**
	 * Creates singleton object for this class and initiates setup, for the first time. 
	 * 
	 * @return BalconyTwo's singleton object
	 */
	public static BalconyTwo getVenueLevel(){
		synchronized (BalconyTwo.class) {
			if(venueLevel == null){
				venueLevel = new BalconyTwo();
			}
		}
		return venueLevel;
	}
	...
	/**
	 *  Initializes, collection of seats in BalconyTwo class. 
	 */
	private static void createBalconyTwoSeatsList(){
		for(Character c = 'A' ; c < 'A'+rows; c++){
			for(int i=1 ; i<= seatsPerRow ; i++){
				Seat seat = new Seat(c + "#" + i, "white");
				balconyTwoSeatsList.add(seat);
			}
		}
	}
	
	/**
	 * Provides all the seats of the BalconyTwo class.
	 * @return a collection of all {@link Seat}
	 */
	private Vector<Seat> getAllSeats(){
		return balconyTwoSeatsList;
	}
